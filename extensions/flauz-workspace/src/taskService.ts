/*---------------------------------------------------------------------------------------------
 *  flauz-workspace — src/taskService.ts
 *
 *  Envelope ownership for `.flauz/tasks.json` (flauz.tasks/v0): bootstrap, canonical
 *  git-diffable serialization (atomic tmp+rename writes), task/id allocation, and the
 *  state machine. Transitions are driven through appendEvent(): an event whose `type`
 *  names a transition is gated by the actor + source status rules in api.TRANSITIONS;
 *  any other event type is appended verbatim as an observational timeline entry.
 *
 *  Strictness (v0): the envelope is validated on load and unknown keys are rejected —
 *  a forward-incompatible envelope must fail loudly, not be silently rewritten.
 *--------------------------------------------------------------------------------------------*/
