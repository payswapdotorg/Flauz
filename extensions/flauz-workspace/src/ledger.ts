 ...
 *
 *  The row hash (never stored) is sha256 over that canonical line — i.e. sha256 of the
 *  stored bytes minus the trailing newline. The next row's `prev` carries it, seq 1
 *  has prev=null. `verify()` recomputes the whole chain and reports the first bad seq.
 *
